(function($, owner) {
	...
	var tk=JSON.parse(stateText).token;
	/**
	预加载页面
	 **/
	owner.getBankName = function(num, callback) {
		var name;
		//	获取银行卡号   使用支付宝的查询接口
		mui.ajax('https://ccdcapi.alipay.com/validateAndCacheCardInfo.json', {
			data: {
...
//				引文缩写
				...
				list.forEach(function(item, index) {
					if(item.value == name) {
						callback(true, data.bank,item.text)
					}
				})
			},
	...
	//	提交绑定新卡信息
	owner.addNewCard = function(carInfo,callback) {
//		alert(1)
		var data=carInfo||{}
		mui.ajax( owner.baseUrl+'/Data/api/Bank/BindingStaffBank', {
			data: data,
			//			dataType:'json',//服务器返回json格式数据
			type: 'post', //HTTP请求类型
			timeout: 10000, //超时时间设置为10秒；
